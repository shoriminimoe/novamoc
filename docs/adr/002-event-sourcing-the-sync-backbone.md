...
**Scope.** Event sourcing applies to synchronized data: assets, maintenance records, and their user-defined field values. The schema (ADR-008) is *not* event-sourced — it is server-authoritative current state with a command-grain audit log. The schema change log is append-only and shares some operational shape with the data event log (per-tenant `seq`, append-only, the basis of the per-tenant version number) but is not folded into the schema projection: the projection holds current state directly, and the log records what happened. There is no LWW, no HLC, and no offline write path for schema. Keeping the schema mechanism out of event-sourcing's scope means data-side machinery (HLC drift bound, LWW fold) does not bear on schema mutations, and the schema-version gate on data events (ADR-009) is not self-referential.
