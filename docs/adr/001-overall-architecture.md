...
The remaining ADRs in this series record specific decisions within this architecture:

- **ADR-002:** Event sourcing for the sync backbone
- **ADR-003:** SQLite/WASM on the client
- **ADR-004:** SQLite on the server (with Litestar)
- **ADR-005:** Schema-as-data with JSON projections, not EAV, for reads
- **ADR-006:** Hybrid Logical Clocks for event ordering
- **ADR-007:** Per-field Last-Write-Wins as the projection fold
- **ADR-008:** Server-authoritative schema
- **ADR-009:** Mandatory schema upgrade before sync resumes
- **ADR-010:** Hand-rolled sync engine
