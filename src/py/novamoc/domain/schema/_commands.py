...
class SchemaCommand(StrEnum):
    ...
    Six verbs (create / activate / deactivate / update / clear / delete)
    prefixed by entity kind. ``create_*`` requires a full payload and
    creates a new entity. ``activate_*`` takes an empty payload and only
    activates an existing-but-deactivated row (or no-ops on an already-
    active one). ``clear`` applies to fields only — types do not have a
    field-value notion. Validity of a (command, payload) pair is enforced
    at the API decoder; storage carries the command as plain TEXT so
    adding new entity kinds or commands is a domain change, not a
    migration.
    """
    ...
    CREATE_ASSET_TYPE_FIELD = "create_asset_type_field"
    ...
    CREATE_MAINTENANCE_RECORD_TYPE = "create_maintenance_record_type"
